...
const App = () => {
  ...
  const [hasAutoGeneratedBriefing, setHasAutoGeneratedBriefing] = useState(false);
  ...
  const generateVoiceBriefing = async () => {
    ...
    try {
      ...
      if (result.marked_read_ids && result.marked_read_ids.length > 0) {
        setMails(prev => {
          const next = (prev || []).map(m => {
            if (result.marked_read_ids.includes(m.id)) {
              return {
                ...m,
                is_read: true,
                labels: (m.labels || []).filter(label => label !== 'UNREAD')
        ...
        setDashboardStats(prev => ({
          ...prev,
          unread_emails: Math.max(0, (prev?.unread_emails || 0) - result.marked_read_ids.length)
        }));
        setStatsData(prev => ({
          ...prev,
          voice_briefings_sent: (prev.voice_briefings_sent || 0) + 1
        }));
      }
  ...
  useEffect(() => {
    if (mails && Array.isArray(mails) && mails.length > 0 && !hasAutoGeneratedBriefing && !voiceGenerated && !isGeneratingVoice) {
      const unreadMails = mails.filter(m => !m.is_read);
      if (unreadMails.length > 0) {
        setHasAutoGeneratedBriefing(true);
        generateVoiceBriefing();
      }
    }
  }, [mails, hasAutoGeneratedBriefing, voiceGenerated, isGeneratingVoice]);
        ...
        {(activeTab === 'Dashboard' || activeTab === 'dashboard') && (
          <div className="dashboard-container" style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              >
                <span style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '500', color: 'var(--text-primary)' }}>
                  {isGeneratingVoice ? '🎙️ Generating voice briefing in background...' : voiceGenerated ? '🎙️ Morning voice briefing is ready!' : '🎙️ Get voice briefing on WhatsApp'}
                </span>
                <span style={{ color: '#F0A500', fontSize: '12px', fontWeight: '600' }}>
                  {voiceGenerated ? 'Listen Now →' : 'Tap to schedule →'}
                </span>
              </div>
              <button 
                onClick={() => setShowVoiceModal(true)} 
                style={{
                  background: voiceGenerated ? '#1AAB8A' : isGeneratingVoice ? '#21262D' : '#F0A500',
                  color: voiceGenerated || isGeneratingVoice ? 'white' : '#0D1117',
                  border: 'none',
                  borderRadius: '8px',
                  padding: '10px 16px',
                  fontSize: '13px',
              ...
              >
                {isGeneratingVoice ? 'Summarizing...' : voiceGenerated ? '▶ Play Voice Note Summary' : 'Generate Voice Note'}
              </button>
            </div>
          </div>
        )}
            ...
            <div style={{ background: '#161B22', border: '1px solid #30363D', borderRadius: '16px', padding: '28px', width: '420px', maxWidth: '90%' }}>
              ...
              {voiceGenerated && (
                <div style={{ background: '#0D1117', borderRadius: '8px', padding: '12px' }}>
                  ...
                  {voiceScript && (
                    <div style={{ marginTop: '16px', background: '#0D1117', border: '1px solid #21262D', borderRadius: '8px', padding: '12px', maxHeight: '120px', overflowY: 'auto', textAlign: 'left' }}>
                      <div style={{ fontSize: '11px', color: '#8B949E', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '6px', letterSpacing: '0.5px' }}>Briefing Script Summary</div>
                      <div style={{ fontSize: '13px', color: '#E6EDF3', lineHeight: '1.5', whiteSpace: 'pre-wrap' }}>
                        {voiceScript}
                      </div>
                    </div>
                  )}
